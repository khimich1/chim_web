...
**Статус:** частично согласован (§1.6 AI-советчик — адаптация `RAG_chemistry`; §14 UI redesign — визуальный язык согласован по референсам)  
...
| **Test/worksheet modal** | Карточка с `×`, индикатором прогресса (pill `N%`/«Шаг N из M»), полями ответа | Stepik-тесты |
...
| Тесты (Stepik) | Одно задание на экран и так mobile-friendly; кнопки «Проверить/Подсказка/Разбор» — крупные, sticky; прогресс сверху |
| ДЗ / списки | Карточки в одну колонку; таблицы → карточный режим |
