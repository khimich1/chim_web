...
**Статус:** частично согласован (§1.6 AI-советчик — адаптация `RAG_chemistry`; §14 UI redesign — визуальный язык согласован по референсам; §1.3.1 — step-dots + возобновление сессии; §1.3.3 — справочник таблицы Менделеева при тестах; AC-6.2 — sources только из учебника, v0.7.3)  
...
- Текст подсказки — если `hint_used`, показать без повторного запроса.
- Текст разбора — доступен на проверенных шагах без повторной проверки (кнопка «Разбор» активна).
...
| Тесты (Stepik) | Одно задание на экран; **StepProgressDots** сверху (горизонтальный скролл при 28 шагах); кнопки «Проверить/Подсказка/Разбор» — крупные, sticky; FAB «Таблица Менделеева» не перекрывает «Далее» |
