...
3. **MVP implement = DeepSeek (OpenAI-compat) + фабрика.** Адаптер `gigachat` — **следующий инкремент** (контракт/ветка в фабрике может быть `NotImplemented` / явная 503 с понятным текстом).
4. **Health API:** добавить `llm_configured: bool`. Поле `openai_configured` **сохранить** с тем же значением (deprecated alias), чтобы не ломать frontend/OpenAPI за один PR. Сообщение UI можно смягчить («LLM API key…»), не обязательно в том же PR.
...
9. **Дефолты при `LLM_PROVIDER=deepseek`:**  
   `LLM_BASE_URL=https://api.deepseek.com` (или `/v1` — как требует установленный `langchain-openai`),  
   `LLM_MODEL=deepseek-chat` (или актуальный alias вендора на момент реализации; зафиксировать в `.env.example`).
...
| Frontend | Минимально: health flag / текст (опционально в том же или следующем PR) |
...
frontend/components/tutor/...           # опционально: текст про LLM key
...
def build_chat_llm(settings: Settings) -> BaseChatModel:
    ...
    if provider == "gigachat":
        raise NotImplementedError("GigaChat adapter — next increment")
...
# openai_api_key / openai_model — legacy; property llm_api_key effective = LLM or OPENAI
```
...
| Уровень | Что |
|---------|-----|
| Unit | Фабрика: `deepseek` → `ChatOpenAI` с ожидаемым `base_url`; unknown provider → error; gigachat → NotImplemented (или skip до инкремента) |
...
| Ситуация | Поведение |
|----------|-----------|
| Нет ключа | 503, detail без утечки внутренних stack; user-msg не в БД |
| `LLM_PROVIDER=gigachat` до адаптера | 503 с явным «GigaChat ещё не подключён» (или отказ при старте settings — выбрать в PLAN) |
...
|-----------|------------|--------|
| **1** | Settings `LLM_*` + aliases; `build_chat_llm` (deepseek/openai); wire graph/service; health `llm_configured`; `.env.example`; tests | pytest tutor + ручной DeepSeek smoke |
...
1. Точный `LLM_BASE_URL`: `https://api.deepseek.com` vs `.../v1` — проверить на версии `langchain-openai` в `requirements` при implement.
2. Имя модели по умолчанию на дату реализации (`deepseek-chat` vs актуальный V4 alias) — зафиксировать в `.env.example` после smoke.
3. Нужно ли в инкременте 1 менять UI-текст «OPENAI_API_KEY не задан» → нейтральный?
