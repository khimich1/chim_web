...
- Ingestion в Chroma; формат документов с metadata (`topic`, `chunk_title`, `chunk_idx`).
...
| **A4** | **Hybrid retrieval (срез 2b)** — keyword ∪ embeddings (pgvector или Chroma), rerank top-10 → top-4 | Низкий recall на синонимах/перефразировании (сейчас только keyword) | `services/rag/` (`vectorstore.py` уже есть как заготовка) | Средний / Высокая |
...
**Порядок реализации (надёжность):** A3 → A2 → A5 → A1 → A6 → A4 → A7
(сначала дешёвые анти-галлюцинационные проверки, затем solve-pipeline, затем hybrid RAG).
...
```
Срез 16-1: Анти-галлюцинации — A3 (порог) + A2 (citation guard) + O1 (логи)   ← быстрый, высокий эффект
Срез 16-2: Solve-pipeline v1.5 — A1 + validation.py (порт RAG_chemistry)
...
Срез 16-6: Hybrid retrieval 2b (A4) + eval-набор O2
Срез 16-7: UX — streaming (U1) + markdown (U2); профиль → PostgreSQL (A7)
```
