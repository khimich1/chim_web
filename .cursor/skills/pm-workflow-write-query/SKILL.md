...
description: "Generate SQL queries from natural language — supports BigQuery, PostgreSQL, MySQL, and more. End-to-end PM workflow for Cursor (chains multiple pm skills). Use when the user asks for /write-query or a full write-query workflow."
disable-model-invocation: true
---

# PM Workflow: write-query

> Cursor adaptation of Claude Code `/write-query`. Invoke explicitly:
> «Следуй skill pm-workflow-write-query» или `@.cursor/skills/pm-workflow-write-query/SKILL.md`

## Cursor notes

- Slash commands (`/write-query`) недоступны — используй этот workflow skill.
...
Read and follow `.cursor/skills/sql-queries/SKILL.md`:
